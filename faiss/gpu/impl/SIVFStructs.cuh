...
// This allows threads in a single Warp to process a Slab in parallel without
...
/**
 * SlabMetadata
 * This is a lightweight structure for managing linked list relationships and
 * the Bitmap.
 *
 * We do not store the float* codes here; instead, they are placed in a massive
 * global memory pool. This structure stores management metadata only.
 */
struct SlabMetadata {
    
    // Linked list pointer: Index of the next Slab in the same inverted list.
    // Using an int index instead of a raw pointer facilitates Host/Device
    // copying/debugging and consumes only 4 bytes.
    int next_slab_idx;

    // Core of Bitmap-based Lazy Eviction.
    // Since CAPACITY is 32, we can perfectly use a uint32_t to represent the
    // validity of all slots. 1 = Valid, 0 = Deleted (Lazy Deleted)
    uint32_t validity_bitmap;

    // Tracks the count of valid vectors currently stored in this Slab (used for
    // fast statistics).
    int valid_count;

    __device__ __host__ SlabMetadata()
        : next_slab_idx(SIVF_NULL_SLAB), validity_bitmap(0), valid_count(0) {}
};
