 ...
 *
 * @note This is a global kernel to allow direct launch configurations (Grid/Block)
 * independent of the host class structure.
 */
...
SlabManager::SlabManager(
  ...
  // zero-initialization (memset), and the kernel launch occur in strict sequence
  // on the GPU without blocking the host CPU.
  auto stream = res->getDefaultStream(device);

  // =========================================================
  // GPU Memory Allocation (Async via Stream)
  // =========================================================
  // The resize() calls below trigger underlying allocations (e.g., cudaMalloc) 
  // and are ordered on the provided stream to ensure serialization.

  // 1. Slab Metadata Pool (Headers)
  // Stores control information for each slab (e.g., num_vectors, next_slab_idx).
  // Mapping: 1-to-1 correspondence with physical slabs.
  ...
  // calculation.
  slab_data_.resize((size_t)slab_pool_size * SIVF_SLAB_CAPACITY * dim, stream);

  // 3. Address Translation Table (Indirection Layer)
  // Maps a logical user vector ID [0, max_vectors) to its physical location
  // (Slab ID + Offset). This indirection enables efficient updates and deletes.
...
///
/// @details This function acts as a bridge between Host (CPU) resource management and 
///          Device (GPU) execution context. It extracts raw pointers from the host-side 
///          RAII containers (e.g., device_vectors) and packages them into a POD 
///          (Plain Old Data) struct.
///
/// @note This operation is "Zero-Copy" regarding GPU memory allocation. It only performs 
///       a shallow copy of the pointer addresses (64-bit integers) and scalar configurations.
///       The returned object is designed to be passed by value to CUDA __global__ kernels.
///
/// @return A fully initialized SlabManagerDevice object containing valid GPU pointers.
SlabManagerDevice SlabManager::getDeviceView() {
  ...
  dev.slab_metadata = slab_metadata_.data();    ///< Ptr to Slab Metadata (Header info)
  dev.slab_data = slab_data_.data();            ///< Ptr to Vector Data (Payload)
  dev.address_table = address_table_.data();    ///< Ptr to Address Table (Indirection layer)
  dev.free_list = free_list_.data();            ///< Ptr to Memory Pool Stack (Free IDs)
  dev.free_list_top = free_list_top_.data();    ///< Ptr to Stack Pointer (Atomic counter)

  // 3. Context Configuration (Scalars)
  // @note These scalars define the physical boundaries for the kernel.
  dev.slab_pool_size = (int)slab_pool_size_;    ///< Physical limit of the memory pool
