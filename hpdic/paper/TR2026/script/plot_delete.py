...
# ==========================================
# Baseline (Faiss): Consistent ~202.2 ms for 10k vectors
raw_baseline = [202.2, 202.2, 202.2] 

# SIVF (Ours): 0.681ms, 0.683ms, 0.668ms
...
# Calculate Speedup
speedup_val = lat_means[0] / lat_means[1]  # approx 298x

# Throughput calculation (vectors per second)
# 10000 vectors / (time_ms / 1000)
...
# ==========================================
# 2. Plotting Setup (Paper Quality)
# ==========================================
# Use Type 42 fonts (TrueType) for editable text in papers
plt.rcParams['pdf.fonttype'] = 42
plt.rcParams['ps.fonttype'] = 42
plt.rcParams['font.family'] = 'sans-serif' # Sans-serif usually looks cleaner in charts
plt.rcParams['font.size'] = 11
plt.rcParams['axes.linewidth'] = 1.2

# Save directory (Update this to your actual paper path)
save_dir = os.path.expanduser('~/ElasticIVF/hpdic/paper/TR2026/figures/') 
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

# Create figure: Wider (9x4.5) to fit double column paper nicely
...
# Set Y-limit to make room for annotations (0.1ms to 1000ms)
...
# Draw arrow from Baseline down to SIVF to show reduction
...
# Move Y-axis label to right for better readability
